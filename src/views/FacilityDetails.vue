...
import { isFacilityStaffParty, useFacilityMutations, useFacilityTypes, useFacilityGroups, useFacilityGroupTypes, useFacilityDetail, useFacilityIdentificationTypes, usePartyQueries, useFacilityOrderCounts, useGeocoding } from '@/composables/useFacilities';
...
const { fetchGeocode } = useGeocoding();
...
async function fetchPostalCodeByGeoPoints() {
  ...
  try {
    const resp = await fetchGeocode(payload);
...
async function generateLatLong() {
  ...
  try {
    const resp = await fetchGeocode({ json: { query: `postcode: ${query}` } });
