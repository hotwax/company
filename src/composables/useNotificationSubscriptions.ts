...
export function useNotificationSubscriptions() {
  ...
  const topics = computed<TopicSummary[]>(() => {
    ...
    for (const row of state.subscriptions) {
      ...
      if (!summary) {
        ...
        summary = { ...parsed, eventName: eventNameById.value[parsed.enumId] || parsed.enumId, userIds: [] };
        byTopic.set(row.topic, summary);
      }
      if (row.userId && !summary.userIds.includes(row.userId)) summary.userIds.push(row.userId);
    }
  ...
  const facilityCount = computed(() => new Set(topics.value.map((topic) => topic.facilityId)).size);

  const userCount = computed(
    () => new Set(topics.value.flatMap((topic) => topic.userIds)).size
  );
