...
/** Keys already eventSourcesRequested, so a re-render or a cache sync cannot queue the same lookup twice. */
const eventSourcesRequested = new Set<string>();
...
async function resolveEventSources(lookups: InventoryEventSourceLookup[], opts: { fanOut?: boolean } = {}): Promise<void> {
  ...
  for(const lookup of lookups) {
    ...
    if(eventSourcesRequested.has(key) || pending.has(key)) {continue;}
  ...
  for(const [key, lookup] of pending) {
    ...
    try {
      ...
      eventSourcesRequested.delete(key);
    }
  }
}
