...
| Superseded wrapper idea | ProductStore-scoped Shopify job setup wrapper |
| Current Company direction | Do not add a ProductStore-specific Shopify job wrapper for the cold-start preview. Company now reads existing `ProductStore`, `ShopifyShop`, `SystemMessageRemote`, `ServiceJob`, and `DataManagerConfig` records directly, then uses the existing service-job CRUD/clone APIs for selected setup actions. |
...
| Job family | Previous wrapper idea | Status | Current primitives | Setup writes |
| --- | --- | --- | --- | --- |
| Shopify sales order sync, fallback/batch | ProductStore-scoped order-import wrapper | Superseded for Company preview | Draft job `queue_ShopifyOrderSync` queues `ShopifyOrderSync` messages. Draft job `sync_ShopifyOrderHistory` uses `BULK_ORDER_HISTORY`. Both eventually stage orders through the same DataManager order import path. Historical import must carry `newOrderSync.launchDate` so pre-launch open orders are imported as historical/no-inventory work instead of live fulfillment work. | Company clones draft jobs to the selected shop/product store with existing service-job APIs, sets `systemMessageRemoteId`, launch/history parameters, and checks `SYNC_SHOPIFY_ORDER` plus `BULK_ORDER_HISTORY` DataManager configs. |
| Shopify sales order sync, realtime SQS | ProductStore-scoped realtime-SQS wrapper | Superseded for Company preview; AWS provisioning remains separate | Existing consumer job `consume_ShopifyOrders_SQS` calls `co.hotwax.shopify.order.SqsOrderImport.consume#SQSOrderMessages`. It reads EventBridge messages from SQS, resolves the Shopify remote from the message shop domain, stages orders, and deletes consumed messages. | Company updates the existing consumer job with `queueName` and `systemMessageRemoteId`. AWS EventBridge/SQS provisioning is still an explicit setup decision, not hidden behind a ProductStore wrapper. |
| Inventory sync to Shopify | ProductStore-scoped inventory-sync wrapper | Superseded for Company preview | Draft job `resetShopifyInventoryQoh` queues `ResetInventoryQoh`. Inventory SECAs in the Shopify bridge post inventory updates only when `ProductStoreSetting SHOPIFY_INV_SYNC=Y`. The bridge service uses Shopify `inventorySetQuantities` with `name=on_hand`. | Company clones/configures `resetShopifyInventoryQoh`, sets `systemMessageRemoteId`, and validates Shopify location mapping/readiness from existing records. |
...
Superseded backend wrapper ideas:

| Endpoint | Status | Purpose |
| --- | --- | --- |
| ProductStore-scoped Shopify job status wrapper | Superseded for Company preview | Replaced by Company-side readiness composition from existing `ProductStore`, `ShopifyShop`, `SystemMessageRemote`, `ServiceJob`, and `DataManagerConfig` records. |
| ProductStore-scoped Shopify draft-job clone wrapper | Superseded for Company preview | Replaced by existing `admin/serviceJobs` clone/update calls scoped to selected job families. |
| ProductStore-scoped order-import setup wrapper | Superseded for Company preview | Replaced by existing service-job update calls for fallback/history order jobs. |
| ProductStore-scoped inventory-sync setup wrapper | Superseded for Company preview | Replaced by existing service-job update calls plus ProductStoreSetting writes where needed. |
| ProductStore-scoped realtime-order SQS wrapper | Superseded for Company preview | Replaced by existing service-job update calls for the selected SQS consumer. |
| Realtime-order AWS resource provisioning endpoint | Future gap | Optional later endpoint to provision EventBridge/SQS resources if we decide Moqui should create AWS infrastructure. |
