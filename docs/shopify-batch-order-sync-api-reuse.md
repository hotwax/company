...
| Adaptive refresh | `useLiveDashboard` visibility/in-flight/stale-data mechanics (that composable was deleted 2026-07-28; the worker owns cadence) | None needed | Yes | Existing Product Sync cadence/lifecycle differs | Small Order Sync polling composable: 10s active, 60s idle, manual refresh, Ionic leave stop |
