...
it('searches the chosen field with a contains operator and selects whole rows', async () => {
  vi.mocked(api).mockReset();
  vi.mocked(api).mockResolvedValue({ data: rows } as any);
  ...
  expect(vi.mocked(api).mock.lastCall?.[0]).toMatchObject({
    params: { shopId: 'shop', orderName_op: 'contains', orderName_ic: 'Y', orderName: '#10', orderByField: '-orderDate' },
  });
...
it('sends no ignore-case flag when searching an id field', async () => {
  ...
  await flushPromises();
  const params: any = vi.mocked(api).mock.lastCall?.[0].params;
...
it('passes the status and date filters through as Moqui search params', async () => {
  ...
  await flushPromises();
  expect(vi.mocked(api).mock.lastCall?.[0]).toMatchObject({
    params: { statusId: 'ORDER_APPROVED', orderDate_from: '2026-05-01 00:00:00', orderDate_thru: '2026-06-01 23:59:59' },
  });
});

it('reports more results only when the page comes back full', async () => {
  vi.mocked(api).mockReset();
  vi.mocked(api).mockResolvedValue({ data: Array.from({ length: 21 }, (_, i) => ({ orderId: String(i), orderName: `#${i}` })) } as any);
  const w = mount(Picker, { props: { shopId: 'shop', modelValue: [] } });
  w.findComponent(IonSearchbar).vm.$emit('ionInput', { detail: { value: '#' } });
  await flushPromises();
  expect(w.text()).toContain('More results');
  expect(vi.mocked(api).mock.lastCall?.[0].params.pageSize).toBe(21);
});
...
it('ignores stale results when a newer query completes first', async () => {
  ...
  vi.mocked(api)
    ...
    .mockResolvedValueOnce({ data: [{ orderId: 'new', orderName: 'New result' }] } as any);
  const w = mount(Picker, { props: { shopId: 'shop', modelValue: [] } });
  ...
  await flushPromises();
  resolveOld({ data: [{ orderId: 'stale', orderName: 'Stale result' }] });
  await flushPromises();
