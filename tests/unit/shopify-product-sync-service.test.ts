...
describe("shopify product sync service", () => {
  ...
  test("fetchDashboardSummary returns shared sync state and unsynced count without reloading system messages", async () => {
    ...
    const service = await loadService(async (request: ApiCall) => {
      ...
      if (request.url === "admin/systemMessages" && request.params?.statusId === "SmsgProduced") {
        return {
          data: {
            systemMessages: [
              {
      ...
      if (request.url === "admin/systemMessages") {
        return {
          data: {
            systemMessages: [
              {
    ...
    assert.equal(calls.filter((call) => call.url === "admin/systemMessages").length, 2);
